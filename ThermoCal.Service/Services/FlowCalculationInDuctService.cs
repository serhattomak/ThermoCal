...
public class FlowCalculationInDuctService : IFlowCalculationInDuctService
{
    public Task<CustomResponseDto<FlowCalculationInDuctResponseDto>> CalculateFlowCalculationInDuctAsync(FlowCalculationInDuctRequestDto request)
    {
        var response = CalculateFlowCalculationInDuct(request);

        return Task.FromResult(CustomResponseDto<FlowCalculationInDuctResponseDto>.Success(200, response));
    }

    private FlowCalculationInDuctResponseDto CalculateFlowCalculationInDuct(FlowCalculationInDuctRequestDto request)
    {
        ...
        return new FlowCalculationInDuctResponseDto
        {
            QDot = qDot,
            WDot = wDot,
            MDot = mDot,
